...
if response.status_code == 200:
    ...
    try:
        ...
        sheet = wb['Vertical Product'] #wb.active  # Or wb['Sheet1']
...
rename_df = format_df.select(
    ...
    F.col("Product Group").alias("Product_Group"),
    F.col("Product Group Description").alias("Product_Group_Description"),
    F.col("Product Category").alias("Product_Category"),
    F.col("Account Assignments Group").alias("Account_Assignments_Group"),
    F.col("Business Units").alias("Product_Business_Unit")
    ).filter(
        ( (F.col("Product").isNotNull())  )
        ).orderBy(F.col("Product").asc())
    
display(rename_df)

# COMMAND ----------

load_data(rename_df, HIVE_SAP_4_HANA, "s4h_product_key_lookup")