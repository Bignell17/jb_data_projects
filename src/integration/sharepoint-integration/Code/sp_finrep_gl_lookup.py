...
if response.status_code == 200:
    ...
    try:
        ...
        sheet = wb['GL Codes'] #wb.active  # Or wb['Sheet1']
...
rename_df = (format_df.select(
    F.col('GL_Code').cast(IntegerType()).alias("GL_Code"),
    'GL_Name',
    F.trim(F.col('Vertical PL mapping')).alias("Vertical_PL_Mapping"),
    F.col('Business Unit').alias("GL_Business_Unit")
    ).filter(
        ((F.col("GL_Code").isNotNull()) | (F.col("GL_Code") != "")))
    .orderBy(F.col("GL_Code").asc())
    )

# COMMAND ----------

load_data(rename_df, HIVE_SAP_4_HANA, "s4h_gl_key_lookup")
