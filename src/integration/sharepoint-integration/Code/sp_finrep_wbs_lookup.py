...
if response.status_code == 200:
    ...
    try:
        ...
        sheet = wb['WBS'] # wb.active  # Or wb['Sheet1']
...
rename_df = format_df.select(
    F.col("WBS per SAP").alias("WBS_per_SAP"),
    F.col("Cost Centre").alias("WBS_Cost_Centre"),
    "Investment_Vertical",
    F.col("Business Case?").alias("WBS_Business_Case")
)

# COMMAND ----------

load_data(rename_df, HIVE_SAP_4_HANA, "s4h_wbs_key_lookup")
